/*
 * $Log: PipeLine.java,v $
 * Revision 1.47.4.1  2007-11-15 15:18:54  europe\L190409
 * fixed commitOnState handling for externally started transactions
 *
...
public class PipeLine {
	public static final String version = "$RCSfile: PipeLine.java,v $ $Revision: 1.47.4.1 $ $Date: 2007-11-15 15:18:54 $";
...
//	
	...
	protected PipeLineResult processPipeLine(String messageId, String message, PipeLineSession pipeLineSession, boolean doTransaction) throws PipeRunException {
		try {
			...
			if (result==null) {
				mustRollback=true;
				log.warn("received null result for messageId ["+messageId+"], transaction (when present and active) will be rolled back");
			} else {
				if (StringUtils.isNotEmpty(getCommitOnState()) && !getCommitOnState().equalsIgnoreCase(result.getState())) {
					mustRollback=true;
					log.warn("result state ["+result.getState()+"] for messageId ["+messageId+"] is not equal to commitOnState ["+getCommitOnState()+"], transaction (when present and active) will be rolled back");
				}
			}
			if (doTransaction) {
