...
public void configure() throws ConfigurationException{
	super.configure();
	// was voorheen public void inlezen_adios() {
    	...
    	BufferedReader bufinput = new BufferedReader(new InputStreamReader(url.openStream()));
 		...
 		while (line != null && !waarde.equals(recordIdentifier)) {
 			StringTokenizer st = new StringTokenizer(line,"{};= \n");
 		 	if (st.countTokens()>=1) {
 				waarde = st.nextToken();
 				if (!waarde.equals(recordIdentifier)) waarde = waarde.substring(3);
 				if (st.hasMoreTokens()) {
...
/*
	String firstToken = new StringTokenizer(v.asString()).nextToken();
	if (firstToken.startsWith("<")  {
	}
*/	
	if (DIRECTION_BACKWARD.equalsIgnoreCase(getDirection())) {
        result = makeAdios(v.asXmlInputSource());
    } else {
 	...
 	try {
 		...
 		while (st1.hasMoreTokens()) {
 			...
 			while (st2.hasMoreTokens()){
 				...
 				if (st2.hasMoreTokens()) waarde = st2.nextToken();else waarde="NVT";
 				waarde =waarde.trim();
 		    	val = nummer2rubriek.get(labelnr);

 				if (val==null) {// label not found in hashtabel, so check for other format.
 					...
 					val = nummer2rubriek.get(labelnr);
 					if (!(recordnr == "")) valrec = nummer2record.get(recordnr);

 					//  label
 					index="";
 					if (val==null) {  // als label niet gevonden dan is het een type: record , rubriek combinatie bv 74,41
 						StringTokenizer st4 = new StringTokenizer(labelnr,"[]");
 						labelnr = st4.nextToken();
 						if (st4.hasMoreTokens()) index= st4.nextToken();
 						val = nummer2rubriek.get(labelnr);
 					}
 					...
 					recindex="";
 					if (!(recordnr=="")) {
 							if (valrec==null) {  // als label niet gevonden dan is het een type: record , rubriek combinatie bv 74,41
 							StringTokenizer st5 = new StringTokenizer(recordnr,"[]");
 							recordnr = st5.nextToken();
 							if (st5.hasMoreTokens()) recindex= st5.nextToken();
 							valrec = nummer2rubriek.get(recordnr);
 						}
 					}
				...
				*/
 					if (val!=null) {
	 					...
	 					rubriek.addAttribute("naam", val.toString());
 						...
 						if (!(recordnr=="")) {
							rubriek.addAttribute("recordnr", recordnr);
	 						rubriek.addAttribute("record", valrec.toString());
 						}
 						if (!(recindex=="")) {
