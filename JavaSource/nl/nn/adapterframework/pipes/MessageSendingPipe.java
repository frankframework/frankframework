...
public PipeRunResult doPipe(Object input, PipeLineSession session) throws PipeRunException {
    ...
    try {
        ...
        if (getSender().isSynchronous()) {
	        log.info(getLogPrefix(session)
                + "sending message to ["+ getSender().getName()+ "] synchronously");
        }
		else {
        	correlationID = Misc.createUUID();
		     log.info(getLogPrefix(session)
                + "sending message to ["+ getSender().getName()+ "]");
		}

        result = getSender().sendMessage(correlationID, (String) input);
        ...
        log.info(getLogPrefix(session) + " result message [" + result + "]");
        return new PipeRunResult(getForward(), result);
	} catch (TimeOutException toe) {
        log.warn(getLogPrefix(session)+"timeout occured");
		return new PipeRunResult(findForward("timeout"), getResultOnTimeOut());
...
public ISender getSender() {
	...
	protected void setListener(ICorrelatedPullingListener listener) {
        ...
        log.debug(
            "pipe ["+getName()+" registered listener ["+ listener.toString()+ "]");
    }
/**
 * Insert the method's description here.
 * Creation date: (27-10-2003 17:01:22)
