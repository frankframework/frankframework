...
public class ConvertToLarvaAction implements CustomReportAction {
	...
	public CustomReportActionResult handleReports(List<Report> reports) {
		...
		Path dir = Paths.get(
				appConstants.getString(appConstants.getString("dtap.stage", null).equals("LOC") ? "ibistesttool.directory" : "log.dir", null),
				"autogenerated"
		);
	...
	private static class Scenario {
		...
		private static final Set<String> ignorePropNames = new HashSet<>(Arrays.asList(
		...
		public Scenario(Report report, Path baseDir, String pipelineName, int uuid) {
			...
			if (!checkScenarioDirNotInUse(scenarioDirPath) || !tryCreateCommonFile(commonFilePath) || !tryLoadCommonProps(commonFilePath)) {
				return;
			}
			...
			if (pipelineClassName != null && pipelineClassName.startsWith(PACKAGE_PREFIX)) {
				senderListenerPackageName = PACKAGE_PREFIX;
			}
			...
			try {
				Files.write(scenarioFilePath, scenarioPropertiesToString().getBytes());
				newFiles.add(scenarioFilePath);

				if (!newFiles.contains(commonFilePath)) {
					addModifiedFile(commonFilePath, new String(Files.readAllBytes(commonFilePath)));
				}
				Files.write(commonFilePath, commonPropertiesToString().getBytes());
		...
		private void processCheckPoints(List<Checkpoint> checkpoints, String adapterName, Path scenarioDirPath, String scenarioDirPrefix) {
			...
			for (Checkpoint checkpoint : checkpoints) {
				...
				if (checkpoint.getLevel() == 1 && checkpoint.getType() == Checkpoint.TYPE_INPUTPOINT && checkpoint.getName().startsWith("SessionKey ")) {
					...
					if (checkpoint.getLevel() == skipUntilEndOfSenderLevel && checkpoint.getType() == Checkpoint.TYPE_ENDPOINT && checkpointName.equals(skipUntilEndOfSenderName)) {
						...
						scenarioPropertiesMap.put(
								"step" + stepCounter + ".stub." + queueName + ".write",
				...
				} else if (checkpoint.getType() == Checkpoint.TYPE_INFOPOINT && allowedPipesWithSenders.contains(extractSimpleClassName(checkpoint.getSourceClassName()))) {
					...
					try {
						...
						stubbedPipe = tp.transform(XmlUtils.stringToSourceForSingleUse(checkpoint.getMessage()));
				...
				} else if (checkpoint.getType() == Checkpoint.TYPE_ENDPOINT && !senderPipeStack.isEmpty() && checkpoint.getName()
						.equals(senderPipeStack.peek().getName())) {
					senderPipeStack.pop();
				}
			}
		...
		private boolean createInputOutputFile(Path folder, String fileName, String message) {
			...
			try {
				if (messageFile.createNewFile()) {
					...
					Files.write(messageFile.toPath(), message.getBytes());
				} else {
					handleError();
					return false;
				}
		...
		private String propertiesToString(TreeMap<String, String> props, UnaryOperator<String> getPropGroup, Integer maxPropNameLength) {
			...
			for (Map.Entry<String, String> entry : props.entrySet()) {
				...
				propStrings.add((isNewPropGroup ? "\r\n" : "")
						+ propName
						+ StringUtils.repeat(" ", Math.max(0, maxPropNameLength - propName.length()))
						+ " = "
						+ value);
			}
			return String.join("\r\n", propStrings);
		...
		private String cleanupModifiedFiles() {
			...
			originalFiles.forEach((path, original) -> {
				try {
					Files.write(path, original.getBytes());
				} catch (IOException e) {
			...
			if (errors.isEmpty()) {
				return "&nbsp;&nbsp;&nbsp;&nbsp;Changes were reverted successfully";
			}
