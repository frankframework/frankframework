...
*/
package org.frankframework.ibistesttool.storage.database;
...
public class DatabaseStorage extends JdbcFacade implements LogStorage, CrudStorage {
...
//	protected @Setter @Getter @Inject @Autowired DbmsSupport dbmsSupport;
	...
	public void store(Report report) throws StorageException {
		...
		if (getDbmsSupport().autoIncrementKeyMustBeInserted()) {
		...
		if (getDbmsSupport().autoIncrementKeyMustBeInserted()) {
			query.append(", " + getDbmsSupport().autoIncrementInsertValue("SEQ_" + getTable()));
		}
		query.append(")");
...
String testQuery = "select max(storageId) from " + getTable();
List<Integer> storageIds = jdbcTemplate.query(testQuery, (rs, rowNum) -> rs.getInt(1));
System.out.println("New storageId: " + storageIds);
	...
	protected void buildMetadataQuery(int maxNumberOfRecords, List<String> metadataNames, List<String> searchValues,
		...
		query.append("select" + getDbmsSupport().provideFirstRowsHintAfterFirstKeyword(maxNumberOfRecords));
		String rowNumber = getDbmsSupport().getRowNumber(metadataNames.get(0), "desc");
		...
		if (StringUtils.isNotEmpty(rowNumber)) {
			...
			query.append(") where " + getDbmsSupport().getRowNumberShortName() + " < ?");
			args.add(maxNumberOfRecords + 1);
			argTypes.add(Types.INTEGER);
		}
		query.append(" order by ");
		query.append(metadataNames.get(0) + " desc");
		query.append(getDbmsSupport().provideTrailingFirstRowsHint(maxNumberOfRecords));
	}
