...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
	...
	<xsl:template match="/">
		<destinations>
			<xsl:for-each-group select="XMI/*[name()='JMSProvider']" group-by="factories[contains(@id,'JMSConnectionFactory')]/@jndiName">
				<xsl:sort select="current-grouping-key()"/>
				<connectionFactory>
					<xsl:attribute name="jndiName"><xsl:value-of select="current-grouping-key()"></xsl:value-of></xsl:attribute>
					<xsl:for-each select="factories[contains(@id,'JMSDestination')]">
