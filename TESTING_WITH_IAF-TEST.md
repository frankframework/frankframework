...
#### Database
To prevent problems with data transactionality, we will be using an Oracle database rather than an H2 database. WeAreFrank! has created a Docker-based solution for running an Oracle database. See our [instructions](test/src/main/tools/setupDb/Oracle/README.txt).
...
## 3. In ibis-adapterframework-test, select Oracle database
...
## 4. Configure the Larva tests

In ``catalina.properties``, please also set a few properties that `ibis-adapterframework-test` needs. Under Windows, they are:

    dtap.stage=LOC
    log.dir=c:/temp
    log.dir.match=(?i)c:[\\\\/]temp
    jdbc.dbms.default=oracle-docker
    larva.windiff.command=c:/Program Files (x86)/WinMerge/winmergeu   

Property `larva.windiff.command` needs to point to your Windiff or WinMerge installation. If you installed it elsewhere, you need a different value for this property. If you do not set this property, you cannot use Windiff or WinMerge from within Larva, which complicates analysing your test results, but the success or failure of the tests is not affected.
