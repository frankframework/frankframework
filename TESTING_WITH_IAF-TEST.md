...
* [jakarta.management.j2ee-api](https://mvnrepository.com/artifact/jakarta.management.j2ee/jakarta.management.j2ee-api)
* [jakarta.jms-api](https://mvnrepository.com/artifact/jakarta.jms/jakarta.jms-api)
* [jakarta.transaction-api](https://mvnrepository.com/artifact/jakarta.transaction/jakarta.transaction-api)
* [jakarta.activation-api](https://mvnrepository.com/artifact/jakarta.activation/jakarta.activation-api)

If you want to use Queuing or a DBMS other than H2, you need to ensure the corresponding JDBC drivers are in place:

* For MariaDB: [mariadb-java-client](https://mvnrepository.com/artifact/org.mariadb.jdbc/mariadb-java-client)
* For MySQL: [mysql-connector-java](https://dev.mysql.com/downloads/connector/j/)
* For MSSQL: [mssql-jdbc](https://docs.microsoft.com/en-us/sql/connect/jdbc/download-microsoft-jdbc-driver-for-sql-server)
* For Oracle: [ojdbc8](https://www.oracle.com/database/technologies/appdev/jdbc-ucp-183-downloads.html)
* For PostgreSQL: [postgresql](https://jdbc.postgresql.org/download/postgresql-42.7.7.jar)
* For ActiveMQ (Classic): [activemq-client](https://mvnrepository.com/artifact/org.apache.activemq/activemq-client)
* For ActiveMQ Artemis: [artemis-client](https://mvnrepository.com/artifact/org.apache.activemq/artemis-jms-client)
...
Download the latest [LTS of Node.js](https://nodejs.org/en) needed to build the frontend.

## 2. Tomcat configuration - Eclipse

The module's test scenarios can be run manually with the Larva testtool. This will be done within a frankframework-test instance running on your Tomcat server. To make this possible...
...
4. In the Project Explorer, right-click the frankframework-test module and select Properties. Go to Deployment Assembly, press [ Add... ]. Select Folder, press [ Next ]. Select the src/main/configurations folder, and press Finish. In the text field right of your new src/main/configurations item, enter `WEB-INF/classes/configurations`. _(might not be necessary anymore)_
...
parallel, so you can for instance run multiple debug sessions side by side to compare system-behavior in a test in different branches.
...
* Some parts of the frankframework-test module rely on proprietary modules. To tell Maven that it should download these modules, go to Window > Preferences > Maven > User Settings. If you already have a _settings.xml_ file, press the "Open file" link. Otherwise, browse to _C:/Users/(your name)/.m2/_ and create a _settings.xml_ file. Edit the file by adding your own repository or the [Frank!Framework Nexus repository](https://nexus.frankframework.org/content/groups/private/) as [mirror](https://maven.apache.org/guides/mini/guide-mirror-settings.html).
