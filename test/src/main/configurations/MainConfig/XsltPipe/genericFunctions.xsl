...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	exclude-result-prefixes="java func frankext cal" version="1.0">
	...
	<func:function name="frankext:changeDate">
		<xsl:param name="date"/>
	...
	<func:function name="frankext:convertDate">
		<xsl:param name="from" />
		<xsl:param name="to" />
		...
		<xsl:variable name="result">
			<xsl:choose>
				...
				<xsl:when test="$strValue != ''">
					<xsl:variable name="formatterFrom" select="java:java.text.SimpleDateFormat.new($from)"/>
					<xsl:variable name="formatterTo" select="java:java.text.SimpleDateFormat.new($to)"/>

					<xsl:variable name="date" select="java:parse($formatterFrom, $strValue)"/>
					<xsl:variable name="tempValue" select="java:format($formatterFrom, $date)"/>
					<xsl:choose>
	...
	<func:function name="frankext:replaceCharsInString">
		...
		<xsl:choose>
			<xsl:when test="contains($stringIn,$charsIn)">
				<func:result
					select="concat(concat(substring-before($stringIn,$charsIn),$charsOut), frankext:replaceCharsInString(substring-after($stringIn,$charsIn),$charsIn,$charsOut))" />
			</xsl:when>
			<xsl:otherwise>
