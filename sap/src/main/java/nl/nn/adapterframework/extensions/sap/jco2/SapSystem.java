...
public class SapSystem extends GlobalListItem  implements ISapSystem, JCO.ServerStateChangedListener  {
	...
	@Override
	@IbisDoc({"4", "alias to obtain userid and password", " "})
	...
	@Override
	@IbisDoc({"5", "userid used in the connection", " "})
	...
	@Override
	@IbisDoc({"6", "passwd used in the connection", " "})
	...
	@Override
	@IbisDoc({"2", "name of the SAP-application server to connect to", " "})
	public void setGwhost(String string) {
		gwhost = string;
	}

	@Override
	@IbisDoc({"7", "Language indicator", "NL"})
	public void setLanguage(String string) {
		language = string;
	}

	@Override
	@IbisDoc({"3", "Mandant i.e. 'client'", "100"})
	...
	@Override
	@IbisDoc({"1", "maximum number of connections that may connect simultaneously to the SAP system", "10"})
	...
	@Override
	@IbisDoc({"10", "number added to systemNr to find corresponding RFC service number", "3300"})
	...
	@Override
	@IbisDoc({"9", "trace level (effective only when logging level is debug). 0=none, 10= maximum", "0"})
