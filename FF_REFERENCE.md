# FF! Reference user manual for contributors

## Code documentation

The FF! Reference provides reference information for Frank developers as explained in [CONTRIBUTING.md](./CONTRIBUTING.md). It is produced by a Doclet that assembles the FF! Reference from the Java sources. You can control the FF! Reference using custom Javadoc tags and using FF! Reference-related Java annotations. These annotations and tags are summarized in the table below:
...
| `@ff.excludeFromType` | `@ExcludeFromType` | Class                                   | Omit as element of specific FF! Reference types (e.g. ITransactionalStorage). Annotation is inherited.                                                                                                                                                                                                 |
...
| `@ff.reintroduce`     | `@Reintroduce`     | Attribute or child setter               | Used to change the order of config children or attributes; see FF! Reference's README file for details.                                                                                                                                                                                                |
| -                     | `@Label`           | Java Annotation                         | When the target annotation is placed on a Java class, a label is added in the FF! Reference webapp. The value of the label is the `value()` field of the target annotation. The name of the label comes from the `@Label` annotation within the definition of the target annotation, attribute `name`. |
...
| `{@inheritDoc}`       |                    | Attribute setter or child setter        | Inherits documentation from a parent class or attribute, if it exists. Additional documentation can be added to supplement the documentation provided by the parent.                                                                                                                                   |
| `{@inheritClassDoc}`  |                    | Class                                   | Inherits documentation from a parent class or interface, if it exists. Additional documentation can be added to supplement the documentation provided by the parent.                                                                                                                                   |
...
The properties file follows a specific format, so it can be scraped to be used as a datasource for the FF! Reference.
