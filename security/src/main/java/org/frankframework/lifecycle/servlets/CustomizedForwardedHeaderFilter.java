/*
 * Copyright 2002-present the original author or authors, 2025 WeAreFrank!.
 *
...
public class CustomizedForwardedHeaderFilter extends OncePerRequestFilter {
	...
	private final boolean allowForwardedHeadersPassthrough;

	public CustomizedForwardedHeaderFilter(boolean allowForwardedHeadersPassthrough) {
		this.allowForwardedHeadersPassthrough = allowForwardedHeadersPassthrough;
	}
	...
	protected void doFilterInternal(@Nonnull HttpServletRequest request, @Nonnull HttpServletResponse response,
		...
		try {
			wrappedRequest = new ForwardedHeaderExtractingRequest(allowForwardedHeadersPassthrough, request);
			wrappedResponse = new ForwardedHeaderExtractingResponse(response, wrappedRequest);
		}
		catch (Throwable ex) {
	...
	private static class ForwardedHeaderRemovingRequest extends HttpServletRequestWrapper {
		...
		public ForwardedHeaderRemovingRequest(boolean allowForwardedHeadersPassthrough, HttpServletRequest request) {
			super(request);
			this.allowForwardedHeadersPassthrough = allowForwardedHeadersPassthrough;
			this.headerNames = headerNames(allowForwardedHeadersPassthrough, request);
		}

		private static Set<String> headerNames(boolean allowForwardedHeadersPassthrough, HttpServletRequest request) {
			Set<String> headerNames = Collections.newSetFromMap(new LinkedCaseInsensitiveMap<>(Locale.ROOT));
			Enumeration<String> names = request.getHeaderNames();
			while (names.hasMoreElements()) {
				String name = names.nextElement();
				if (allowForwardedHeadersPassthrough || !FORWARDED_HEADER_NAMES.contains(name)) {
	...
	private static class ForwardedHeaderExtractingRequest extends ForwardedHeaderRemovingRequest {
		...
		ForwardedHeaderExtractingRequest(boolean allowForwardedHeadersPassthrough, HttpServletRequest servletRequest) {
			super(allowForwardedHeadersPassthrough, servletRequest);
