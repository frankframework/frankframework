...
- Only the H2 database driver is provided in the Docker image. This means that drivers for other databases (e.g. PostgreSQL, Oracle, MSSQL, MySQL/MariaDB) are not provided any more and should be provided by the user when using those databases. There are no JMS drivers provided anymore either.\
