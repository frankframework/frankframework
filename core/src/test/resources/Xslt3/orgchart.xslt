...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"  xmlns:j="http://www.w3.org/2013/XSL/json">
	...
	<xsl:param name="header" select="'headerDefault'"/>
	<xsl:param name="sessionKey" select="'sessionKeyDefault'"/>
	<xsl:param name="sessionKeyGlobal" select="'sessionKeyGlobalDefault'"/>
	
	<xsl:template match="/">
		<j:map>
			<j:string key="header"><xsl:value-of select="$header"/></j:string>
			<j:string key="sessionKey"><xsl:value-of select="$sessionKey"/></j:string>
			<j:string key="sessionKeyGlobal"><xsl:value-of select="$sessionKeyGlobal"/></j:string>
			<j:array key="data">
