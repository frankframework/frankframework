...
public class XsdUtilsTest {
	...
	public void targetNameSpaceAddingEmpty() throws XMLStreamException {
		...
		/*
		 * assertEquals(in, XsdUtils.targetNameSpaceAdding(in, "")); assertEquals(in,
		 * XsdUtils.targetNameSpaceAdding(in, null));
		 */
	}
	...
	public void targetNameSpaceAddingAndDefaultNamespace() throws XMLStreamException, IOException, SAXException {
		...
		/*
		 * IOUtils.copy(XsdUtils.targetNameSpaceAdding(new
		 * ByteArrayInputStream(testXsd.getBytes()),
		 * "http://wub2nn.nn.nl/GetIntermediaryAgreementDetails"), result); Diff diff =
		 * XMLUnit.compareXML(expected, result.toString()); assertTrue(diff.toString() +
		 * " " + result.toString(), diff.identical());
		 */
	}
