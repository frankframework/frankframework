...
public class MigratorTest extends TransactionManagerTestBase {
	...
	public void testSimpleChangelogFile() throws Exception {
		...
		assertEquals(1, messageKeeper.size()); //Configuration startup message + liquibase update
		assertEquals("Configuration ["+getConfiguration().getName()+"] LiquiBase applied [2] change(s) and added tag [two:Niels Meijer]", messageKeeper.getMessage(0).getMessageText());
