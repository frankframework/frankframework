...
public class FrankConfigLayoutFlowGeneratorTest {

	private static final String BASE_DIR = "/FlowDiagram/ConfigLayout/";
	...
	void test(String directory) throws Exception {
		...
		IFlowGenerator generator = new FrankConfigLayoutFlowGenerator();
		...
		generator.generateFlow(testFile, byteArrayOutputStream);

		String expectedFlow = TestFileUtils.getTestFile(testFileDir + EXPECTED_FILENAME);
		TestAssertions.assertEqualsIgnoreCRLF(expectedFlow, byteArrayOutputStream.toString(StandardCharsets.UTF_8));
		generator.destroy();
	}
