...
<!--		<xsl:copy-of select="$preproccessedConfiguration"/>-->
	...
	<!-- For parallel senders, make sure forwards from pipes within are printed inside the pipe too -->
	<xsl:template match="sender[@className='nl.nn.adapterframework.senders.ParallelSenders']" mode="preprocess">
		<xsl:variable name="defaultCopyActions">
			<xsl:call-template name="defaultCopyActions"/>
		</xsl:variable>
		<xsl:copy>
			<xsl:copy-of select="$defaultCopyActions"/>
		</xsl:copy>
		<xsl:copy-of select="$defaultCopyActions/forward"/>
	</xsl:template>
	...
	<xsl:template match="*" mode="sort">
		...
		<!--When walking through the pipeline via forwards, pipes might be duplicated, so sort them by errorHandling keep only the first one of each.
			If there is an errorHandling and a default version of a pipe this will keep the default one.-->
		<xsl:choose>
