...
<!--		<xsl:copy-of select="$preproccessedConfiguration"/>-->
			...
			<xsl:variable name="forwards">
				<xsl:for-each select="forward">
					<pair>
						...
						<xsl:copy>
							<xsl:attribute name="errorHandling" select="$errorHandling or @name = $errorForwards or $target/@shape = 'errorhandling'"/>
							<xsl:copy-of select="@*|*"/>
						</xsl:copy>
	...
	<xsl:template name="styleElement">
		...
		<xsl:choose>
			<xsl:when test="$style = 'endpoint' and count(sender) = 1">
				<xsl:attribute name="shape" select="($pipeTypes/*[name()=current()/sender/@className],$style)[1]"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:attribute name="shape" select="$style"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
...
<!--	</xsl:template>-->
	...
	<xsl:variable name="shapeStartMap">
		<field shape="endpoint">((</field>
		<field shape="validator">([</field>
		<field shape="wrapper">[[</field>
		<field shape="translator">(</field>
		<field shape="splitter">[/</field>
		<field shape="router">{{</field>
		<field shape="session">[/</field>
		<field shape="errorhandling">[</field>
		<field shape="database">[(</field>
	</xsl:variable>
	...
	<xsl:template name="createMermaidElement">
		...
		<xsl:value-of select="($shapeStartMap/field[@shape = current()/@shape],'(')[1]"/>
		<xsl:value-of select="$text"/>
		<xsl:if test="$subText != ''">
			<xsl:text>&lt;br/></xsl:text>
			<xsl:value-of select="$subText"/>
		</xsl:if>
		<xsl:value-of select="($shapeEndMap/field[@shape = current()/@shape],')')[1]"/>
		<xsl:text>:::</xsl:text>
		<xsl:choose>
