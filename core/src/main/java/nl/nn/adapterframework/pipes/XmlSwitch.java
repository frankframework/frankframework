...
public class XmlSwitch extends AbstractPipe {
	...
	public void configure() throws ConfigurationException {
		...
		if (StringUtils.isNotEmpty(getXpathExpression())) {
			...
			transformerPool = TransformerPool.configureTransformer0(getLogPrefix(null), getConfigurationClassLoader(), getNamespaceDefs(), getXpathExpression(), null, "text", false, getParameterList(), getXsltVersion());
		} 
		else {
			if (!StringUtils.isEmpty(getServiceSelectionStylesheetFilename())) {
				try {
