...
public class MailSender extends SenderWithParametersBase {
		...
		if (paramList == null) {
			...
			try {
				...
				if (pv != null) {
					...
					log.debug("MailSender [" + getName() + "] retrieved subject-parameter ["
							+ subject + "]");
				}
				...
				if (pv != null) {
					...
					log.debug("MailSender [" + getName() + "] retrieved message-parameter ["
							+ message + "]");
				}
				...
				if (pv != null) {
					...
					log.debug("MailSender [" + getName() + "] retrieved charset-parameter ["
							+ charset + "]");
				}
				...
				throw new SenderException(
						"MailSender [" + getName() + "] got exception determining parametervalues",
						e);
			}
			messageInMailSafeForm = sendEmail(from, subject, threadTopic, message, messageType,
					messageBase64, charset, recipients, attachments);
		}
	...
	protected String sendEmail(String input, ParameterResolutionContext prc)
		...
		try {
			...
			if (attachmentsElement != null)
				attachments = retrieveAttachments(
						XmlUtils.getChildTags(attachmentsElement, "attachment"), prc);

		} catch (DomBuilderException e) {
			throw new SenderException("exception parsing [" + input + "]", e);
		}

		return sendEmail(from, subject, threadTopic, message, messageType, messageBase64, charset,
	...
	protected String sendEmail(String from, String subject, String threadTopic, String message,
			...
			Collection<Recipient> recipients, Collection<Attachment> attachments)
			throws SenderException {
	...
	@IbisDoc({
			"timeout (in milliseconds). used for socket connection timeout and socket i/o timeout",
			"20000" })
