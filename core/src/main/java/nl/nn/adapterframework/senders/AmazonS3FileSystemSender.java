...
public class AmazonS3FileSystemSender extends FileSystemSender<S3Object, AmazonS3FileSystem> {
	...
	private List<String> specificActions = Arrays.asList("createBucket", "deleteBucket", "copy",
			"restore");
	...
	public void configure() throws ConfigurationException {
		...
		if (getAction().equalsIgnoreCase("createBucket")
				&& getFileSystem().isForceGlobalBucketAccessEnabled()
				&& (StringUtils.isEmpty(getFileSystem().getBucketRegion())
						|| !AmazonS3FileSystem.AVAILABLE_REGIONS
								.contains(getFileSystem().getBucketRegion())))
			throw new ConfigurationException(
					" invalid bucketRegion [" + getFileSystem().getBucketRegion()
							+ "] please use following supported regions "
							+ AmazonS3FileSystem.AVAILABLE_REGIONS.toString());
		else if (getAction().equalsIgnoreCase("copy")) {
			if (StringUtils.isEmpty(getFileSystem().getDestinationBucketName()) || !BucketNameUtils
					.isValidV2BucketName(getFileSystem().getDestinationBucketName()))
				throw new ConfigurationException(" invalid or empty destinationBucketName ["
						+ getFileSystem().getDestinationBucketName()
			...
			if (getFileSystem().isStorageClassEnabled()
					&& (StringUtils.isEmpty(getFileSystem().getStorageClass())
							|| !AmazonS3FileSystem.STORAGE_CLASSES
									.contains(getFileSystem().getStorageClass())))
				throw new ConfigurationException(
						" invalid storage class [" + getFileSystem().getStorageClass()
				...
				&& (StringUtils.isEmpty(getFileSystem().getTier())
