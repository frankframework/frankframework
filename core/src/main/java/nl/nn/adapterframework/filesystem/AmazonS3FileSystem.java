...
public class AmazonS3FileSystem implements IFileSystem<S3Object> {
		...
		if (exists(f)) {
			if (isFolder(f)) {
				...
				throw new FileSystemException(
						"trying to remove file [" + f.getKey() + "] which is a file instead of a directory");
			}
		} else {
			throw new FileSystemException("trying to remove file [" + f.getKey() + "] which does not exist");
		}
	...
	*/
	...
	public String copyObject(String bucketName, String fileName, String destinationBucketName,
		...
		try {
			...
			if (!s3Client.doesObjectExist(destinationBucketName, destinationFileName)) {
				CopyObjectRequest copyObjectRequest = new CopyObjectRequest(bucketName, fileName, destinationBucketName,
						destinationFileName);
	...
	public String restoreObject(String bucketName, String fileName, int experationInDays) throws SenderException {
		Boolean restoreFlag;
		try {
			bucketDoesNotExist(bucketName);
			fileDoesNotExist(bucketName, fileName);
			RestoreObjectRequest requestRestore = new RestoreObjectRequest(bucketName, fileName, experationInDays)
					.withTier(tier);
			...
			log.error("Failed to perform [restore] action, and restore object with fileName [" + fileName
					+ "] from Amazon S3 Glacier");
			throw new SenderException("Failed to perform [restore] action, and restore object with fileName ["
