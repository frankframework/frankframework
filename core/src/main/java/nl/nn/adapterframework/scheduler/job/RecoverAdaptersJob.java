...
public class RecoverAdaptersJob extends JobDef {
		...
		for (Configuration configuration : ibisManager.getConfigurations()) {
			if(configuration.isActive()) {
				for (Adapter adapter: configuration.getRegisteredAdapters()) {
					countAdapter++;
					handleAdapter(adapter, countAdapterStateStarted, countReceiver, countReceiverStateStarted);
				}
			}
		}
	...
	private void handleAdapter(Adapter adapter, int countAdapterStateStarted, int countReceiver, int countReceiverStateStarted) {
