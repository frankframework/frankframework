 ...
 * </ul>
 *
 * <p><b>Configuration </b><i>(where deviating from JmsTransactionalStorage)</i><b>:</b>
 * <table border="1">
...
public class EsbJmsTransactionalStorage extends JmsTransactionalStorage {
	...
	public void configure() throws ConfigurationException {
		...
		try {
			URL exceptionLogURL = ClassUtils.getResourceURL(this, exceptionLogString);
	...
	public String storeMessage(String messageId, String correlationId, Date receivedDate, String comments, String label, Serializable message) throws SenderException {
		...
		try {
			...
			ObjectMessage msg = session.createObjectMessage((Serializable)logRequest);
			String returnMessage = send(session,getDestination(),msg);
			log.debug("sent message ["+logRequest+"]");
	...
	private Map createParameterValues(String messageId, String correlationId, Date receivedDate, String comments, String label, Serializable message) throws JMSException, DomBuilderException, TransformerException, IOException {
		...
		parameterValues.put("senderId", getSlotId()); 
		if (getType().equalsIgnoreCase("E")) {
