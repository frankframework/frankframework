...
public class ClassLoaderURIResolver implements URIResolver {
	...
	public ClassLoaderURIResolver(IHasConfigurationClassLoader classLoaderProvider) {
		if (log.isTraceEnabled()) log.trace("ClassLoaderURIResolver init with classLoaderProvider ["+classLoaderProvider+"]");
	...
	public Resource resolveToResource(String href, String base) throws TransformerException {
		...
		String ref=ref1;
		Resource resource = Resource.getResource(classLoaderProvider, ref, protocol);
		if (resource==null && ref2!=null) {
			...
			ref=ref2;
			resource = Resource.getResource(classLoaderProvider, ref, protocol);
		}
		if (resource==null) {
			String message = "Cannot get resource for href [" + href + "] with base [" + base + "] as ref ["+ref+"]" +(ref2==null?"":" nor as ref ["+ref1+"]")+" protocol ["+protocol+"] classLoaderProvider ["+classLoaderProvider+"]";
