...
public class MySqlDbmsSupport extends GenericDbmsSupport {
	...
	private String convertOracleQuerySelectFromDual(Connection connection, String[] split, boolean resultQuery) throws JdbcException {
	...
	private int retrieveAutoIncrement(Connection connection, String tableName)
		...
		if (countAutoIncrement == 0) {
			throw new JdbcException(
					"could not find auto_increment column for table [" + tableName + "]");
		} else if (countAutoIncrement > 1) {
			throw new JdbcException(
					"could not have multiple auto_increment columns for table [" + tableName + "]");
		}
	...
	private boolean sequenceExists(Connection connection, String sequenceName) {
		...
		try {
			...
			if (log.isDebugEnabled())
				log.debug("exception checking for existence of [" + sequenceName + "] using query [" + query + "]: " + e.getMessage());
			return false;
		}
	}
...
//			String query = "ALTER SEQUENCE " + sequenceName + " RESTART " + startWith;
