...
export class MiscService {
  constructor(private appConstants: AppConstants, private Base64: Base64Service){}

  getServerPath(): string {
    var absolutePath = this.appConstants["server"];
  ...
  compare_version(v1: string | number, v2: string | number, operator?: string): boolean | number | null {
    ...
    var _prepVersion = function (v: string | number) {
      v = ('' + v).replace(/[_\-+]/g, '.');
      v = v.replace(/([^.\d]+)/g, '.$1.').replace(/\.{2,}/g, '.');
      return (!v.length ? [-8] : v.split('.'));
    };
    var _numVersion = function (v: string | number): number {
...
appModule.service('Misc', ['appConstants', 'Base64', MiscService]);
