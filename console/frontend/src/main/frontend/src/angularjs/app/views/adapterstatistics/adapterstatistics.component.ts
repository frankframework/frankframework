...
class AdapterstatisticsController {
  ...
  refreshing = false;
  hourlyStatistics: { labels: string[]; data: number[] } = {
    labels: [],
    data: [],
  };
  stats = [];
  statisticsTimeBoundaries = angular.copy(this.defaults);
  statisticsSizeBoundaries = angular.copy(this.defaults);
  statisticsNames = [];
  dataset = {
    fill: false,
  ...
  refresh() {
    ...
    this.Api.Get("configurations/" + this.Misc.escapeURL(this.configuration) + "/adapters/" + this.Misc.escapeURL(this.adapterName) + "/statistics", (data) => {
      this.stats = data;
