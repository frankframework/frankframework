...
export class AdapterstatisticsService {
  ...
  getStatistics(configurationName: string, adapterName: string){
    return this.http.get<Statistics>(this.appService.absoluteApiPath + "configurations/" + this.Misc.escapeURL(configurationName) + "/adapters/" + this.Misc.escapeURL(adapterName) + "/statistics");
  }
}
